...
public class MainPage {
    ...
    public static By questionAboutCostAndPaymentAccordion = By.xpath(".//div[contains(text(), 'Сколько это стоит?')]");

    //Ответ на вопрос о стоимости и способе оплаты
    public static By answerOnCostAndPayment = By.xpath(".//p[contains(text(), 'рублей') and contains(text(), 'Оплата курьеру')]");

    //Вопрос о возможности арендовать несколько самокатов
    private final By questionAboutOrderSeveralScootersAccordion = By.xpath(".//div[contains(text(), 'несколько самокатов!')]");

    //Ответ на вопрос о возможности арендовать несколько самокатов
    private final By answerOnOrderSeveralScooters = By.xpath(".//p[contains(text(), 'несколько заказов')]");

    //Вопрос о расчете времени аренды
    private final By questionAboutCalcOrderTimeAccordion = By.xpath(".//div[text()='Как рассчитывается время аренды?']");

    //Ответ на вопрос о расчете времени аренды
    private final By answerOnCalcOrderTime = By.xpath(".//p[contains(text(), 'Отсчёт времени')]");

    //Вопрос о возможности заказать самокат прямо на сегодня
    private final By questionAboutOrderScooterTodayAccordion = By.xpath(".//div[contains(text(), 'прямо на сегодня')]");

    //Ответ на вопрос о возможности заказать самокат прямо на сегодня
    private final By answerOnOrderScooterToday = By.xpath(".//p[contains(text(), 'с завтрашнего дня')]");

    //Вопрос о возможности продлить заказ или вернуть самокат раньше
    private final By questionAboutIncreaseOrDecreaseOrderTimeAccordion = By.xpath(".//div[contains(text(), 'продлить заказ')]");

    //Ответ на вопрос о возможности продлить заказ или вернуть самокат раньше
    private final By answerOnIncreaseOrDecreaseOrderTime = By.xpath(".//p[contains(text(), 'если что-то срочное')]");

    //Вопрос о том, привозят ли зарядку вместе с самокатом
    private final By questionAboutBringingChargerAccordion = By.xpath(".//div[contains(text(), 'зарядку вместе с самокатом')]");

    //Ответ на вопрос о том, привозят ли зарядку вместе с самокатом
    private final By answerOnBringingCharger = By.xpath(".//p[contains(text(), 'Зарядка')]");

    //Вопрос о возможности отмены заказа
    private final By questionAboutCancelOrderAccordion = By.xpath(".//div[contains(text(), 'отменить заказ')]");

    //Ответ на вопрос о возможности отмены заказа
    private final By answerOnCancelOrder = By.xpath(".//p[contains(text(), 'пока самокат не привезли')]");

    //Вопрос о возможности привезти самокат за МКАД
    private final By questionAboutBringingScooterBeyondMRRAccordion = By.xpath(".//div[contains(text(), 'за МКАДом')]");

    //Ответ на вопрос о возможности привезти самокат за МКАД
    private final By answerOnBringingScooterBeyondMRR = By.xpath(".//p[contains(text(), 'Москве') and contains(text(), 'Московской области')]");

    //Кнопка "Заказать" вверху страницы
    private final By orderScooterButtonOnTop = By.className("Button_Button__ra12g");

    //Кнопка "Заказать" внизу страницы
    private final By orderScooterButtonOnBottom = By.xpath(".//div[@class='Home_FinishButton__1_cWm']/button[text()='Заказать']");
